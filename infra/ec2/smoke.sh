...
hr "tools";       for t in git python3 uv node npm aws jq rg tmux; do printf "%-8s %s\n" "$t" "$(command -v $t >/dev/null && $t --version 2>&1 | head -1 || echo MISSING)"; done
...
hr "python + uv"; cd "$(mktemp -d)" && uv init -q --name smoke >/dev/null && uv add -q requests >/dev/null && uv run python -c "import requests,sys;print(sys.version.split()[0], requests.__version__)" && cd / && echo "uv project ok"
